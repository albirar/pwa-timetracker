...
import { type DbStorage_Api, IndexDefinition, instantiateDb } from './timetracker-db';
...
export interface ApiCheckRegister {
     ...
     * @return The current state
     */
    refreshState() : Promise<CheckState>;
    /**
    ...
    /**
     * Gets the total time of state CheckedIn for the indicated temporal frame.
     * @param temporalFrame The temporal frame to obtain the total time
     * @return The total time of the temporalFrame
     */
    totalTime(temporalFrame: TemporalFrame) : Promise<number>;
}
...
class ApiCheckRegisterImpl implements ApiCheckRegister {
    ...
    async refreshState(): Promise<CheckState> {
        return new Promise<CheckState>( async (resolve, reject) => {
            var rg : CheckOperationRegister;
            const db = await this.dbCheckAndOpen();

            db.getLastRegister().then((reg:CheckOperationRegister) => {
    ...
    async totalTime(temporalFrame: TemporalFrame): Promise<number> {
        return new Promise<number>(async (resolve, reject) => {
            var totalT : number = 0;
            var n : number;
            var tsStart : number = undefined;
            var lastReg : CheckOperationRegister = undefined;

            this.findOperations(temporalFrame).then((ops) => {
                for(n=0; n < ops.length; n++) {
                    lastReg = ops[n];
                    if(lastReg.checkOperation == CheckOperation.CheckInOperation) {
                        tsStart = lastReg.moment;
                    } else {
                        if(tsStart != undefined) {
                            totalT += (lastReg.moment - tsStart);
                        }
                    }
                }
